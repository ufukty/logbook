...
func NewFixed(start time.Time, length, resolution time.Duration) *Fixed {
	i := &Fixed{
		start:  start,
		length: length,
		res:    resolution,
...
func (c *Fixed) Save(t time.Time, v int) {
...
func (c *Fixed) cellvalue(level, cell int) int {
...
func (c *Fixed) query(from, to, level int) (int, error) {
	if from > to {
...
func (c *Fixed) Query(from, to time.Time) (int, error) {
