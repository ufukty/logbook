...
test -f "$STAGE/provisioning/vpn/terraform.tfstate" && (
  jq -c '.resources.[] | select(.type == "digitalocean_droplet").instances.[]' <"$STAGE/provisioning/vpn/terraform.tfstate" |
    while read -r DROPLET; do
      ...
      scp -i "$STAGE/secrets/ssh/do" "$STAGE/secrets/pki/vpn/ca.crt" "$VPS_SUDO_USER@$IP:ca.crt"
      scp -i "$STAGE/secrets/ssh/do" "$STAGE/secrets/pki/vpn/issued/$SERVERNAME.crt" "$VPS_SUDO_USER@$IP:server.crt"
      scp -i "$STAGE/secrets/ssh/do" "$STAGE/secrets/pki/vpn/private/$SERVERNAME.key" "$VPS_SUDO_USER@$IP:server.key"
      scp -i "$STAGE/secrets/ssh/do" "$STAGE/secrets/pki/vpn-users/crl.pem" "$VPS_SUDO_USER@$IP:crl.pem"
